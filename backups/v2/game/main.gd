...
func _build_golfer() -> void:
	...
	for side in [-1,1]:
		...
		var elbow:=Vector3(side*0.30,0.26,-0.32)
		var hand:=Vector3(side*0.055,0.01,-0.52)
		_limb(shoulder,shoulder.lerp(elbow,0.48),0.13,0.12,c,torso)
		_limb(shoulder.lerp(elbow,0.45),elbow,0.095,0.075,skin,torso)
		_limb(elbow,hand,0.075,0.052,skin,torso)
		course.sphere(hand,0.065,Color("f5f2e4") if side==-1 else skin,torso)
	_limb(Vector3(0,0.02,-0.53),Vector3(0.44,-1.02,-0.75),0.016,0.012,Color("bacbd2"),torso)
	var clubhead=course.sphere(Vector3(0.44,-1.02,-0.75),0.11,Color("1e3544"),torso); clubhead.scale=Vector3(1.35,0.65,0.8)

func _limb(a: Vector3,b: Vector3,r1: float,r2: float,col: Color,parent: Node3D) -> void:
...
func shoot(power: float, error: float=0.0) -> void:
	...
	var swing:=create_tween(); swing.tween_property(torso,"rotation:z",-0.7,0.22); swing.tween_property(torso,"rotation:z",0.0,0.8)
...
func _update_camera(delta: float, instant: bool=false) -> void:
	...
	if state=="menu":
		...
		target=ball_pos+aim_direction()*(1 if putting else 3)
		target.y=course.height_at(target.x,target.z)+0.4
		pos=ball_pos-aim_direction()*(10 if putting else 11)+Vector3(0,8 if putting else 3.3,0)
