...
func _menu() -> void:
	...
	text(Vector2(38,554),"Same clubs. Same stats. All personality.",14,MUTED)
	button(Rect2(38,574,399,60),"LET'S PLAY GOLF    →","play",false,true)
	button(Rect2(38,649,194,40),"How to swing","help")
	button(Rect2(243,649,194,40),"Sound: "+("on" if game.sound_enabled else "off"),"sound")
	...
	panel(Rect2(504,407,740,366),Color(0.065,0.19,0.15,0.94),16)
	text(Vector2(526,439),"PICK YOUR ALTER EGO",13,GOLD,bold)
	for i in range(game.player_count):
		button(Rect2(526+i*170,451,160,33),"P%d  %s"%[i+1,Data.GOLFERS[game.roster[i]][0].split(" ")[0]],"slot:%d"%i,game.active_slot==i)
	for i in range(8):
		var x: float=526+(i%4)*175
		var y: float=498+int(i/4)*120
		var r:=Rect2(x,y,162,109)
		...
		_avatar(Vector2(x+28,y+35),Color(Data.GOLFERS[i][2]),0.7)
		if selected: text(Vector2(x+120,y+25),"P%d"%(game.active_slot+1),13,GOLD,bold)
		var name: Array=Data.GOLFERS[i][0].split(" ")
		text(Vector2(x+12,y+74),name[0],15,CREAM,bold)
		text(Vector2(x+12,y+94),name[1],15,CREAM,bold)
		buttons.append([r,"golfer:%d"%i])

func _avatar(at: Vector2,c: Color,scale_value: float=1.0) -> void:
	draw_circle(at,25*scale_value,c.darkened(0.58))
	draw_circle(at+Vector2(0,17)*scale_value,18*scale_value,c)
	draw_circle(at+Vector2(0,-2)*scale_value,12*scale_value,Color("d9a57d"))
	draw_arc(at+Vector2(0,-5)*scale_value,13*scale_value,PI,TAU,20,c,9*scale_value,true)
	draw_line(at+Vector2(-13,-8)*scale_value,at+Vector2(20,-8)*scale_value,c,5*scale_value,true)
	for x in [-4,4]: draw_circle(at+Vector2(x,-1)*scale_value,1.5*scale_value,INK)
	draw_arc(at+Vector2(0,1)*scale_value,5*scale_value,0.2,2.8,10,INK,1,true)
...
func _game_hud() -> void:
	...
	text(Vector2(295,53),"HOLE %02d / %02d"%[game.hole_index+1,game.round_length],14,GOLD,bold)
	...
	_avatar(Vector2(60,160),Color(Data.GOLFERS[p.golfer][2]),0.78)
	text(Vector2(91,146),"PLAYER %d · %s"%[game.player_index+1,game.relative_score(game.player_index)],12,GOLD,bold)
	...
	text(Vector2(45,688),"Est. distance" if Data.CLUBS[game.club][4] and game.club!=13 else "Tour carry baseline" if game.club!=13 else "Slope affects the roll",11,MUTED)
...
func _swing_pad() -> void:
	...
	text(Vector2(976,602),"SWING STUDIO",11,INK,bold)
	if game.state!="aim":
		center(Rect2(966,623,280,80),"LET IT COOK…",18,INK,bold); return
	var power: float=game.charge if game.keyboard_charge else game.pull
	if game.dragging or game.keyboard_charge:
		text(Vector2(976,635),"%d%%"%roundi(power*100),28,INK,bold)
		panel(Rect2(976,649,112,9),Color("c5a45c"),4)
		panel(Rect2(976,649,112*power,9),INK,4)
		text(Vector2(976,683),"PUSH FORWARD",11,INK,bold)
		text(Vector2(976,704),"Release to strike",11,INK)
	else:
		text(Vector2(976,636),"01  Hold & pull back",13,INK,bold)
		text(Vector2(976,661),"02  Push up & release",13,INK,bold)
		text(Vector2(976,720),"Or hold SPACE, release to hit",11,INK)
...
func _minimap(r: Rect2) -> void:
	...
	var maprect:=Rect2(r.position+Vector2(12,45),Vector2(r.size.x-24,r.size.y-74))
	var pts:=PackedVector2Array()
	for i in range(41):
		var z: float=-game.course.length_m*i/40.0
		pts.append(_map_point(Vector3(game.course.center_x(z),0,z),maprect))
	draw_polyline(pts,Color("708e50"),27,true)
	draw_polyline(pts,Color("97b264"),17,true)
	for e in game.course.ponds:
		var p:=_map_point(Vector3(e.x,0,e.y),maprect)
		draw_circle(p,12,Color("71b2b4"))
	for e in game.course.bunkers: draw_circle(_map_point(Vector3(e.x,0,e.y),maprect),5,Color("ecdbad"))
	var cup:=_map_point(game.course.pin,maprect)
	draw_circle(cup,11,Color("bad17f")); draw_circle(cup,3,INK)
	var bp:=_map_point(game.ball_pos,maprect)
	draw_circle(bp,5,CREAM)
	if game.state=="aim":
		var end:=_map_point(game.marker.position,maprect)
		draw_dashed_line(bp,end,GOLD,1.5,4)
		draw_arc(end,5,0,TAU,20,GOLD,1.5,true)
	text(r.position+Vector2(16,r.size.y-12),"N ↑     V  FULL HOLE",10,MUTED)

func _map_point(p: Vector3,r: Rect2) -> Vector2:
	return Vector2(r.position.x+r.size.x/2+p.x*0.7,r.end.y+ p.z/game.course.length_m*r.size.y)

func _ball_tracker() -> void:
	if game.impact_pending:
		panel(Rect2(464,500,330,42),INK,10)
		center(Rect2(464,500,330,42),"BACKSWING" if game.swing_elapsed<0.55 else "DOWNSWING",16,GOLD,bold)
		return
...
func _scorecard() -> void:
	...
	for i in range(18): center(Rect2(startx+i*cell,267,cell,37),str(i+1),12,CREAM)
	center(Rect2(1103,267,57,37),"TOT",12,GOLD,bold)
	text(Vector2(139,331),"PAR",12,Color("6d795f"),bold)
	for i in range(18): center(Rect2(startx+i*cell,309,cell,31),str(Data.HOLES[i][1]),12,Color("6d795f"))
	for pi in range(game.players.size()):
		var y: float=349+pi*43
		if pi%2==0: panel(Rect2(122,y-1,1038,40),Color("e8e5cf"),4)
		text(Vector2(139,y+25),Data.GOLFERS[game.players[pi].golfer][0],14,INK,bold)
		for i in range(18):
			var value: String="—"
			...
			if i<game.players[pi].scores.size():
				...
				if score<Data.HOLES[i][1]: draw_circle(Vector2(startx+i*cell+cell/2,y+19),14,Color("e9c974"))
	...
	if game.state=="finished":
		...
		button(Rect2(844,593,304,44),"FINAL RESULTS  →" if game.hole_done and game.hole_index+1>=game.round_length else "NEXT TEE  →" if game.hole_done else "BACK TO PLAY","next",false,true)
...
func _help() -> void:
	...
	var rows: Array=[
		...
		["03  MAKE YOUR SWING", "In the gold pad, hold near the top and drag down for power."],
		["04  FOLLOW THROUGH", "Push back up and release. Sideways motion creates draw or fade."],
		["TRACKPAD ALTERNATIVE", "Hold SPACE to build power; release for a straight shot."],
		["READ THE COURSE", "Wind, uphill lies, rough, sand, bounce and spin affect the result."],
		["ON THE GREEN", "Putter scale adapts to distance. The green grid reveals the surface."],
		["HOUSE RULES", "Farthest ball plays next. Water / OB: +1 and replay the shot."],
		["KEEP IT MOVING", "Par + 5 pickup. Skins ties carry; an unresolved final pot expires."]
	]
