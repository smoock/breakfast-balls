...
func shot_comment(override_category: String="") -> String:
	...
	var lines: Dictionary={
		...
		"ROUGH":["A LITTLE EXTRA FIBER","SCRAMBLE SPECIAL","FOUND THE LONG GRASS","TIME TO DIG IN"],
...
func _input(event: InputEvent) -> void:
	if event is InputEventKey and event.pressed and not event.echo:
		...
		if state=="aim" and not help_open:
			...
			if event.keycode==KEY_SPACE: keyboard_charge=true; charge=0; swing_clock=0; path_offset=0
	...
	if event is InputEventMouseButton:
		...
		if event.button_index==MOUSE_BUTTON_LEFT:
			if event.pressed and hud.swing_rect.has_point(event.position):
				dragging=true; drag_start=event.position; drag_current=event.position; peak_y=event.position.y; pull=0; swing_clock=0; path_offset=0
				...
				if dragging:
					...
					if pull>0.06: shoot(pull,error,path_offset)
					dragging=false
				aiming_drag=false
