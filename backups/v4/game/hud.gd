...
func _menu() -> void:
	...
	for i in range(8):
		...
		_avatar(Vector2(x+37,y+31),Color(Data.GOLFERS[i][2]),0.85,i)
...
func _swing_pad() -> void:
	...
	if game.dragging or game.keyboard_charge:
		...
		text(Vector2(976,677),"PATH %+.0f%%"%(game.path_offset*100),11,INK,bold)
		...
		text(Vector2(976,730),"SPACE + A/D also works",10,INK)
